---
name: summary-note
description: 为 Markdown 和 HTML 文章自动创建、更新并维护元数据。根据文件名生成标题，使用文件系统创建日期，从正文生成摘要和标签，并依据父目录设置分类；支持 YAML Front Matter 与 HTML meta 标签，避免重复写入且保持幂等。适用于添加或更新文章 metadata、Front Matter、meta 标签、标题、摘要、分类和标签，以及明确要求的批量文章元数据整理。
---
...
使用文章源文件在当前文件系统中的创建日期，格式为：

```text
YYYY-MM-DD
```

写入文件前先读取文件创建时间（Creation Time / Birth Time），并转换为当前系统时区的日期。

按以下优先级确定 `date`：

1. 能够取得文件创建时间时，使用文件创建日期；已有 `date` 与其不一致时也要更新。
2. 无法取得文件创建时间但已有有效 `date` 时，保留已有日期。
3. 无法取得文件创建时间且不存在有效 `date` 时，使用当前系统日期。

有效日期必须采用 `YYYY-MM-DD` 格式，并且是实际存在的公历日期。

Windows PowerShell 示例：

```powershell
(Get-Item -LiteralPath $articlePath).CreationTime.ToString("yyyy-MM-dd")
```
...
3. 按 `date` 规则读取文件创建日期并更新 `date`。
4. 无法取得文件创建时间时，有效的已有 `date` 优先于当前系统日期。
...
1. 标签已存在时更新其 `content`。
2. 标签不存在时创建。
3. 按 `date` 规则读取文件创建日期并更新 `date`；无法取得文件创建时间时，有效的已有 `date` 优先于当前系统日期。
4. 不得生成相同 `name` 的重复标签。
...
* 不凭空生成文章中不存在的结论。
* 不使用文件修改日期冒充创建日期。
* 能够取得文件创建时间时，不使用元数据添加日期、任务执行日期或已有错误日期代替文件创建日期。
* 不重复插入元数据。
* 不删除未知的 YAML 字段。
* 不删除无关的 HTML `<meta>` 标签。
* 不修改正文标题来配合元数据。
